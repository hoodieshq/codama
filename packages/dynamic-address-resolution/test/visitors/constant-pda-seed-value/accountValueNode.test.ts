...
describe('constant-pda-seed-value: visitAccountValue', () => {
    test('should throw UNEXPECTED_NODE_KIND — accounts are not resolvable in a constant context', () => {
        const visitor = makeConstantVisitor();
        expect(() => visitor.visitAccountValue(accountValueNode('anything'))).toThrow(
